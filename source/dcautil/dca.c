...
static int getSplitParameterValue(char *line, pcdata_t *pcnode, rdkList_t *pchead) {
    ...
    if(strFound != NULL)
    {
        if(strncmp(pcnode->header, "EVT_", 4) == 0)
        {
            ...
            */
            insertPCNode(&pchead, pcnode->pattern, pcnode->header, pcnode->d_type, 0, line, false, NULL);
        }
        else
        {
 ...
 * @retval Return 0 upon success, -1 on failure.
 */
static int processCountPattern(hash_map_t *logSeekMap, char *logfile, rdkList_t *pchead, int pcIndex, rdkList_t **rdkec_head, int *firstSeekFromEOF, int exec_count, bool check_rotated_logs) {
    T2Debug("%s ++in\n", __FUNCTION__);
    char temp[MAXLINE];
 ...
 */
static int processPattern(char **prev_file, char *logfile, rdkList_t **rdkec_head, rdkList_t *pchead, int pcIndex, Vector *grepResultList, hash_map_t* logSeekMap, int *firstSeekFromEOF, int exec_count, bool check_rotated_logs) {
